...
namespace
{
    bnb::logger logging("camera_bin_mocker");
...
void camera_bin_mocker::update_image_content()
{
    ...
    std::fseek(file, static_cast<long>(m_offest), SEEK_CUR); // skip the ordinal number
    std::fread(static_cast<void*>(m_y_plane.data()), 1, m_y_plane.size(), file);
    std::fread(static_cast<void*>(m_uv_plane.data()), 1, m_uv_plane.size(), file);
    fclose(file);
    ...
    format.width = static_cast<uint32_t>(m_width);
    format.height = static_cast<uint32_t>(m_height);
...
{
    logging.warning("The feature to set device by index is unsupported.");
}

void camera_bin_mocker::set_device_by_id(const std::string&)
{
    logging.warning("The feature to set device by id is unsupported.");
}
