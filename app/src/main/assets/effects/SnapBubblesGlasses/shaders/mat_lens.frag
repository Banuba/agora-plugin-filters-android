...
BNB_DECLARE_SAMPLER_VIDEO(12, 13, glfx_VIDEO);
...
void main()
{
    ...
    vec3 I_video = normalize(var_v - var_cam_pos);
    vec3 R_video = reflect(I_video, normalize(var_n));

    vec2 video_color_uv = R.xy;
    video_color_uv.x += 0.5;
    video_color_uv.y += 0.1;
    vec3 additionColor = BNB_TEXTURE_2D(BNB_SAMPLER_2D(glfx_VIDEO), scaleUV(video_color_uv, vec2(0.7))).rgb;
    vec3 color = (kD * diffuse * 1.1 + specular * 0.03 + additionColor * 1.2);
    ...
    bnb_FragColor = vec4(color * 2.0 , opacity * 0.5 ) * 2.0;
    // bnb_FragColor = BNB_TEXTURE_2D(BNB_SAMPLER_2D(glfx_VIDEO), var_uv);
#ifdef GLFX_OCCLUSION
