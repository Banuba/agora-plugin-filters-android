...
void main()
{
    ...
    vec3 color = (kD*diffuse*opacity + specular);
#else
    ...
    for( int i = 0; i != lights.length(); ++i )
    {
        ...
        color += ( kD_light*base/3.14159265*opacity + specular )*radiance[i]*diffuse_factor( N_L, lwrap )/*cN_L*/;
    }
#endif
    ...
    float zoom_factor = mix( 0.4, 1.5, var_zoom_center_d.z );
    vec2 zoom_uv = var_zoom_center_d.xy+(var_bg_uv-var_zoom_center_d.xy)*zoom_factor;
	vec3 bg = texture(glfx_BACKGROUND,zoom_uv).xyz;
	vec3 mixed_color = bg*(1.-opacity) + l2g(color);
    frag_color = vec4(mixed_color,1.);
}
