...
BNB_OUT(6) vec3 var_zoom_center_d;
...
var_bg_uv.y = 1. - var_bg_uv.y;
    ...
    vec4 zoom_view = bnb_MVP * vec4(vec4(0.,0.,0.,1.)*m);
    var_zoom_center_d.xy = (zoom_view.xy/zoom_view.w)*0.5 + 0.5;
    var_zoom_center_d.z = clamp(length(attrib_pos)/40.-0.6, 0., 1. );
}