...
namespace agora::extension
{
    ...
    void BanubaVideoFilter::getVideoFormatWanted(rtc::VideoFrameData::Type& type, rtc::RawPixelBuffer::Format& format)
    {
        type = rtc::VideoFrameData::Type::kRawPixels;
        format = rtc::RawPixelBuffer::Format::kNV12;
    }

    void BanubaVideoFilter::getProcessMode(rtc::IExtensionVideoFilter::ProcessMode& mode, bool& independent_thread)
    {
        mode = rtc::IExtensionVideoFilter::ProcessMode::kAsync;
        independent_thread = false;
    }

    rtc::IExtensionVideoFilter::ProcessResult BanubaVideoFilter::pendVideoFrame(agora_refptr<rtc::IVideoFrame> frame)
    {
        m_video_processor->process_frame(frame);
        return rtc::IExtensionVideoFilter::ProcessResult::kSuccess;
    }

    int BanubaVideoFilter::setProperty(const char* key, const void* buf, size_t buf_size)
    {
